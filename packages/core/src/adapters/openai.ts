/**
 * @module adapters/openai
 * OpenAI provider adapter
 */

import type { ChatMessage } from '../types/index.js';
import type {
  ProviderCapabilities,
  ModelPricing,
  OpenAIMessage,
  NormalizedResponse,
  CountOptions,
} from './types.js';
import { BaseAdapter } from './base.js';

/**
 * OpenAI provider adapter
 *
 * Supports GPT-4o, GPT-4, GPT-3.5-turbo, o1, and related models.
 * Features automatic prompt caching (no configuration required).
 */
export class OpenAIAdapter extends BaseAdapter {
  readonly name = 'openai' as const;

  readonly supports: ProviderCapabilities = {
    caching: true,
    cacheTTL: [5, 10, 60], // 5-10 min default, up to 1 hour off-peak
    streaming: true,
    tools: true,
    multiModal: true,
    extendedThinking: true, // o1 models support reasoning
    maxContextTokens: {
      'gpt-4o': 128000,
      'gpt-4o-mini': 128000,
      'gpt-4-turbo': 128000,
      'gpt-4': 8192,
      'gpt-4-32k': 32768,
      'gpt-3.5-turbo': 16385,
      'o1': 200000,
      'o1-mini': 128000,
      'o1-preview': 128000,
    },
    minCacheTokens: 1024,
  };

  protected readonly defaultModel = 'gpt-4o';

  protected readonly pricing = new Map<string, ModelPricing>([
    ['gpt-4o', {
      model: 'gpt-4o',
      provider: 'openai',
      inputPer1k: 0.0025,
      outputPer1k: 0.01,
      cachedInputPer1k: 0.00125, // 50% discount
      contextWindow: 128000,
      maxOutputTokens: 16384,
    }],
    ['gpt-4o-mini', {
      model: 'gpt-4o-mini',
      provider: 'openai',
      inputPer1k: 0.00015,
      outputPer1k: 0.0006,
      cachedInputPer1k: 0.000075,
      contextWindow: 128000,
      maxOutputTokens: 16384,
    }],
    ['gpt-4-turbo', {
      model: 'gpt-4-turbo',
      provider: 'openai',
      inputPer1k: 0.01,
      outputPer1k: 0.03,
      cachedInputPer1k: 0.005,
      contextWindow: 128000,
      maxOutputTokens: 4096,
    }],
    ['gpt-4', {
      model: 'gpt-4',
      provider: 'openai',
      inputPer1k: 0.03,
      outputPer1k: 0.06,
      contextWindow: 8192,
      maxOutputTokens: 8192,
    }],
    ['gpt-3.5-turbo', {
      model: 'gpt-3.5-turbo',
      provider: 'openai',
      inputPer1k: 0.0005,
      outputPer1k: 0.0015,
      cachedInputPer1k: 0.00025,
      contextWindow: 16385,
      maxOutputTokens: 4096,
    }],
    ['o1', {
      model: 'o1',
      provider: 'openai',
      inputPer1k: 0.015,
      outputPer1k: 0.06,
      cachedInputPer1k: 0.0075,
      contextWindow: 200000,
      maxOutputTokens: 100000,
    }],
    ['o1-mini', {
      model: 'o1-mini',
      provider: 'openai',
      inputPer1k: 0.003,
      outputPer1k: 0.012,
      cachedInputPer1k: 0.0015,
      contextWindow: 128000,
      maxOutputTokens: 65536,
    }],
  ]);

  /**
   * Count tokens using gpt-tokenizer for accuracy
   */
  override async countTokens(text: string, options?: CountOptions): Promise<number> {
    try {
      // Dynamically import gpt-tokenizer for accurate counting
      const { encode } = await import('gpt-tokenizer');
      return encode(text).length;
    } catch {
      // Fallback to estimation if tokenizer not available
      return super.countTokens(text, options);
    }
  }

  /**
   * Count message tokens with OpenAI-specific overhead
   */
  override async countMessages(messages: ChatMessage[], options?: CountOptions): Promise<number> {
    const includeOverhead = options?.includeOverhead ?? true;
    let total = 0;

    try {
      const { encode } = await import('gpt-tokenizer');

      for (const message of messages) {
        total += encode(message.content).length;
        // OpenAI adds ~4 tokens per message for formatting
        if (includeOverhead) {
          total += 4;
        }
      }

      // Additional tokens for the overall prompt structure
      if (includeOverhead && messages.length > 0) {
        total += 3;
      }
    } catch {
      // Fallback to estimation
      return super.countMessages(messages, options);
    }

    return total;
  }

  /**
   * Normalize messages to OpenAI format
   */
  normalizeMessages(messages: ChatMessage[]): OpenAIMessage[] {
    return messages.map(msg => ({
      role: msg.role as OpenAIMessage['role'],
      content: msg.content,
      ...(msg.name && { name: msg.name }),
    }));
  }

  /**
   * Normalize OpenAI response to standard format
   */
  normalizeResponse(response: unknown): NormalizedResponse {
    const res = response as {
      choices?: Array<{
        message?: { content?: string; tool_calls?: Array<{
          id: string;
          function: { name: string; arguments: string };
        }> };
        finish_reason?: string;
      }>;
      usage?: {
        prompt_tokens?: number;
        completion_tokens?: number;
        total_tokens?: number;
        prompt_tokens_details?: { cached_tokens?: number };
      };
      model?: string;
    };

    const choice = res.choices?.[0];
    const usage = res.usage;

    return {
      content: choice?.message?.content ?? '',
      usage: {
        input: usage?.prompt_tokens ?? 0,
        output: usage?.completion_tokens ?? 0,
        cached: usage?.prompt_tokens_details?.cached_tokens,
        total: usage?.total_tokens ?? 0,
      },
      model: res.model ?? 'unknown',
      finishReason: this.mapFinishReason(choice?.finish_reason),
      toolCalls: choice?.message?.tool_calls?.map(tc => ({
        id: tc.id,
        name: tc.function.name,
        arguments: tc.function.arguments,
      })),
      raw: response,
    };
  }

  /**
   * Map OpenAI finish reason to standard format
   */
  private mapFinishReason(reason?: string): NormalizedResponse['finishReason'] {
    switch (reason) {
      case 'stop':
        return 'stop';
      case 'length':
        return 'length';
      case 'tool_calls':
      case 'function_call':
        return 'tool_calls';
      case 'content_filter':
        return 'content_filter';
      default:
        return 'stop';
    }
  }
}

/**
 * Create an OpenAI adapter instance
 */
export function createOpenAIAdapter(): OpenAIAdapter {
  return new OpenAIAdapter();
}
